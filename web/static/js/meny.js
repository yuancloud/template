      ...
      return (function() {
        ...
        function setupContents() {
          ...
          if (supports3DTransforms) {
            ...
            style.position = style.position.match(/relative|absolute|fixed/gi)
              ? style.position
              : 'relative';
        ...
        function open() {
          if (!isOpen) {
            ...
            if (supports3DTransforms) {
              ...
              dom.menu.style[Meny.prefix('transform')] =// ...fall back on JS animation
              menuTransformOpened;
            } else {
        ...
        function onTap() {
          var isOverContent = (config.position === POSITION_T && touchStartY > config.height) || (config.position === POSITION_R && touchStartX < dom.wrapper.offsetWidth - config.width) || (config.position === POSITION_B && touchStartY < dom.wrapper.offsetHeight - config.height) || (config.position === POSITION_L && touchStartX > config.width);
      ...
      return (function() {
        ...
        for (var p in properties) {
          interpolations[p] = {
            ...
            unit: (typeof properties[p] === 'string' && properties[p].match(/px|em|%/gi))
              ? properties[p].match(/px|em|%/gi)[0]
              : ''
          };
        }
